...
int check_struct(char *str, int *error_nb)
{
    ...
    else if(info[0][0] == 't' && info[0][1] == 'r' && info[0][2] == 'a'  && ft_strlen(info[0]) == 3)
    {
        if(nb_word(str) != 2)
            return(11);
        if(check_position(info[1]) == 0)
    ...
    else if(info[0][0] == 'r' && info[0][1] == 'o' && info[0][2] == 't'  && ft_strlen(info[0]) == 3)
    {
        if(nb_word(str) != 2)
            return(12);
        if(check_position(info[1]) == 0)
   ...
   /*  if(info[0][0] == '\n')
        return(1); */
    // ask abt new line
    //segf when new line and space
    //-1is for wrong parsing
    return(-1);
}